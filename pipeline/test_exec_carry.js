...
chk(!/ให้เกินกำลัง/.test(rpt1), 'ข้อมูลไม่เปลี่ยน → ไม่ฟ้องว่าผู้ให้เกินกำลัง (เศษปัดบาทต้องไม่นับ)');
chk(!/ไม่มีในข้อมูลงวดนี้/.test(rpt1), 'ข้อมูลไม่เปลี่ยน → ไม่มีรหัส รพ. หาย');
// ยอด "ยังขาดอยู่" ต้องกระทบยอดกับคำเตือนของปุ่ม ⚡ (ลำพูน 4 แห่ง ~22.8M) ไม่ใช่เลขคนละชุด
const mShort=rpt1.match(/ยังขาดอยู่ (\d+) แห่ง รวม ([\d.]+)M/);
chk(!!mShort, 'รายงานบอกยอดที่ยังขาดเป็นตัวเลขให้กระทบยอดได้');
