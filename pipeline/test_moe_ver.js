...
const A=new Function(code+`;return {exRender,exSimPath,exSolveFor,exNiMo,exMoeMo,exMoeAccMo,exMoeStretchMo,
  exMoeGrpRaw,exMoeCashMo,exXmoeMo,exMoeVer,exPayPct,exMoeVerLab,exMoeLeft,exTopUp,
...
chk(Math.abs(v69-1409.1e6)<1e5, 'Ver69 = 1,409.1 ลบ./เดือน (ค่าที่ยันไว้กับ pipeline)', M(v69));
chk(Math.abs(v68-1391.4e6)<1e5, 'Ver68 = 1,391.4 ลบ./เดือน (เท่าโมเดลกองเศรษฐฯ ที่เคยยืนยัน)', M(v68));
...
chk(Math.abs(res100-1613.56e6)<1e5, 'เงินสำรอง MOE ทั้งเขต = 1,613.56 ลบ. (รวมค่าเช่าแล้ว · ถ้าไม่รวม 1,443.07)', M(res100));
...
for(const [ver,pct,lab] of [['69',100,'MOE.Ver69'],['68',100,'MOE.Ver68'],['69p',40,'MOE.Ver69 + จ่ายหนี้การค้า 40%']]){
  ...
  chk(!err && nTh===14 && main.length===j.hosp.length && badTd===0 && !/undefined|NaN/.test(html+moeBox)
