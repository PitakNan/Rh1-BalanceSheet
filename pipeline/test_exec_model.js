...
// ⚠️ ค่าฐานงวด 256910 (ก.ค. 69) = 80.00M/15 แห่ง — ของเดิม 57.25M/11 เป็นงวด 256909
//    ตัวเลขขยับด้วย 3 เหตุพร้อมกัน อย่าเข้าใจผิดว่ามาจากข้อมูลเดือนใหม่อย่างเดียว:
//    ① ข้อมูลเดินไป ก.ค. (ขาดทุนเดือนเดียว 299.8 ลบ.) · ② เหลือพยากรณ์ 2 เดือนแทน 3
//    ③ คืน bs.niYE/clYE ที่หายไปตอนเดินงวด + ตัวคูณกันนับซ้ำ exYeAdj (0.9 ที่ ด.10)
{ const ok=Math.abs(sum-80.00e6)<1e5&&n===15;
  console.log('  เงินสนับสนุนรวม (เป้า 6):',M(sum),'·',n,'แห่ง  (ต้องเท่า 80.00M/15 แห่ง)',ok?'✅':'❌');
  if(!ok) fail.push(`เงินสนับสนุนรวมเปลี่ยน: ได้ ${M(sum)}/${n} แห่ง ควรเป็น 80.00M/15 แห่ง`); }
