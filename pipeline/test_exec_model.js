...
// ⚠️ ค่าฐานใหม่ 12 ส.ค. 69 = 104.35M/20 แห่ง (เดิม 79.90M/15) — เปลี่ยนเพราะ bs.niProf
//    เจ้าของงานสั่งว่า NI ต้องไม่ใช่ค่าเฉลี่ยต่อเดือน ต้องมาจากงบทดลองและขยับตามเดือนที่เลือก
//    ของเดิม niYE ใช้ "ค่าเดียว" กับ ก.ค./ส.ค./ก.ย. เท่ากันหมด · ของใหม่แยกรายเดือนจริง
//    ส.ค. −578.4 · ก.ย. −537.5 ลบ. (เดิม −519.4 ทั้งคู่) → สะสม 2 เดือนแย่ลง 77 ลบ. = ต้องเติมเงินมากขึ้น
// 12 ส.ค. 69 รอบ 2: +clProf (โปรไฟล์ CL รายเดือน) → 104.35 → 104.95M
// 12 ส.ค. 69 รอบ 4: + 🏦 หักเงินของเขตที่ฝากไว้ 162.2 ลบ. ออกจากเงินสด 3 แห่ง
//   + แก้บั๊ก QR>CR (ปล่อยสินค้าคงเหลือ/ลูกหนี้คืนเกินยอดที่มีจริง) → 104.20 → 107.65M
// 12 ส.ค. 69 รอบ 3: + บรรทัด "การเปลี่ยนแปลงเงินทุนหมุนเวียน" ของงบกระแสเงินสดวิธีทางอ้อม
//   (ลูกหนี้/สินค้าคงเหลือดูดเงินสดไปก่อน) → 104.95 → 104.20M / 20 → 18 แห่ง
//   ยอดรวมลดแต่ backtest **แย่ลง 6 → 10 แห่ง** = โมเดลสมจริงขึ้น ไม่ใช่ผ่อนลง
{ const ok=Math.abs(sum-107.65e6)<1e5&&n===18;
  console.log('  เงินสนับสนุนรวม (เป้า 6):',M(sum),'·',n,'แห่ง  (ต้องเท่า 107.65M/18 แห่ง)',ok?'✅':'❌');
  if(!ok) fail.push(`เงินสนับสนุนรวมเปลี่ยน: ได้ ${M(sum)}/${n} แห่ง ควรเป็น 107.65M/18 แห่ง`); }
...
{ const want='0×30 1×18 2×10 3×14 4×4 5×3 6×6 7×18';   // 12 ส.ค. 69 หลัง niProf+clProf+เงินทุนหมุนเวียน+หักเงินเขตฝาก
