...
// 13 ส.ค. 69: +2 คอลัมน์จำลองหนี้ตามจ่าย (🔬 เจ้าหนี้/ลูกหนี้ สังกัด สธ. เทียบค่าเฉลี่ยตัวเอง · คู่มือ 7.32)
//   ⛔ 2 คอลัมน์นี้เป็นตัวสังเกตการณ์ ไม่เข้าสมการ — ข้อ "สายเลขคณิตปิดตรง" ด้านล่างจึงไม่นับรวม
// 18 ส.ค. 69: +1 คอลัมน์ 💵 "เงินที่ต้องใช้จริง" = max(ส่วนขาดสภาพคล่อง, เงินสนับสนุน)
//   ⛔ เป็นยอดตั้งงบก้อนเดียว ไม่ใช่ผลบวกของ 2 คอลัมน์ซ้าย (เงินก้อนเดียวแก้ทั้ง 2 มิติ = บวกกันแล้วนับซ้ำ)
const NCOL=24+(A.SHOW_TJAR?1:0);
const NEEDC=A.NEEDC;
...
// รอบ 2 (12 ส.ค. 69): +clProf → CL เดินตามทิศทางรายเดือนจริงแทน clMo แบน/clYE บล็อก 3 เดือน
//   เดิมก่อน clProf: [1,119.75] [2,104.35] [3,104.35] [6,152.20] [13,293.55]
...
// รอบ 5 (18 ส.ค. 69): ยันใหม่ +0.05 ทุกเดือนเป้า — ⚠️ **ไม่ได้มาจากการแก้โค้ด** แต่มาจากข้อมูล
//   exec.json ถูก regenerate 16 ส.ค. 69 (ยังไม่ commit) ทำให้ รพ. 11147/11149 เปลี่ยน → ยอดรวมขยับ 1 SV_STEP
//   พิสูจน์แล้วโดยรันสูตรชุดเดียวกันบนข้อมูล 3 รุ่น: HEAD 107.65 · ข้อมูล 16 ส.ค. 107.70 · หลังแก้โค้ด 107.70
//   (การเติมบัญชี p9 และการปัด 2 ตำแหน่งไม่กระทบยอด Solver เลย — กระทบเฉพาะคอลัมน์ส่วนขาด/เครื่องหมาย ✓✗)
//   เดิมรอบ 4: [1,126.00] [2,107.65] [3,115.80] [6,168.85] [13,369.35]
for(const [mmo,want] of [[1,126.05],[2,107.70],[3,115.85],[6,168.90],[13,369.40]]){
  A.setEXST(ST(0,mmo));
