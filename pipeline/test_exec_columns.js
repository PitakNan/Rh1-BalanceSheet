...
for(const mmo of [1,3,6,13]){
  ...
  for(const h of j.hosp){
    ...
    if(!cLeft.includes(wantSub)||!cLeft.includes('กรณีรายรับไม่เป็นไปตามแผน')) badSub++;
  }
...
console.log('━━ สาขาเตือน "เงินสดติดลบ" (เคสสังเคราะห์) ━━');
{
  ...
  chk(plain.includes('กรณีรายรับไม่เป็นไปตามแผน'), 'ยังติดป้ายสมมติฐานให้บรรทัดบนครบ');
