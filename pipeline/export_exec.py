...
MOE_PAY9 = {
    ...
    "5104030205.112", "5104030205.113", "5104030205.117", "5104030205.118",
    "5104030206.101",
    # ── เพิ่ม 18 ส.ค. 69 (เจ้าของงานเคาะ) — ปิดช่องว่างที่ปุ่ม Ver69p เอื้อมไม่ถึง ──
    # ที่มา: ตรวจทั้งเขตงวด 256910 พบว่ากลุ่มที่มีเครดิตเทอมแต่ไม่มีบัญชีใน p9 เลย
    #   = วัสดุทั่วไป 34.90 + ซ่อมแซม 33.23 + จ้างเหมาบริการ 231.76 = 300.88 ลบ./ด. (28% ของกลุ่มเครดิต)
    #   ทำให้ Ver69p ตั้งจ่าย 0% แล้วส่วนขาดยังลงได้แค่ 28.50 ลบ. (ติดเพดาน ไม่ใช่ค่าจริง)
    # ⛔ เจ้าของงานเคาะว่า **กลุ่ม svc (จ้างเหมาบริการ) ยืดไม่ได้ → คงเป็นจ่ายสด ห้ามใส่ p9**
    #    เหตุผล: ผู้รับเหมาต้องจ่ายค่าแรงคนของเขาทุกเดือน (ทำความสะอาด/อาหาร/รถ/รปภ./ซักรีด/Lab/X-Ray)
    # ผลหลังเพิ่ม: ส่วนขาดที่ Ver69p 0% = 23.75 ลบ. / 10 แห่ง (จาก 28.50 / 11 แห่ง)
    # วัสดุทั่วไปใช้ไป (mat) — ซื้อด้วยเครดิตผู้ขาย เหมือนยา/เวชภัณฑ์
    "5104010104.101", "5104010104.102", "5104010104.103", "5104010104.104",
    "5104010104.105", "5104010104.106", "5104010104.107", "5104010104.108",
    "5104010104.109",
    # ซ่อมแซม/บำรุงรักษา (rep) — วางบิลเมื่องานเสร็จ มีเครดิตเทอม
    # ⚠️ 5 ตัวท้าย (.109-.113) เป็น "ค่าจ้างเหมาบำรุงรักษา/ซ่อมแซม" ซึ่งใกล้เคียงกลุ่ม svc
    #    ต่างกันที่เป็นสัญญาบำรุงรักษาตามรอบ (ลิฟท์/แอร์/ครุภัณฑ์การแพทย์) ไม่ใช่ค่าแรงรายเดือน
    #    ถ้า CFO เห็นว่าควรเป็นจ่ายสดเหมือน svc ให้ถอด 5 รหัสนี้ออกได้ (กระทบ < 33.23 ลบ./ด.)
    "5104010107.101", "5104010107.102", "5104010107.103", "5104010107.104",
    "5104010107.105", "5104010107.106", "5104010107.107", "5104010107.108",
    "5104010107.109", "5104010107.110", "5104010107.111", "5104010107.112",
    "5104010107.113",
